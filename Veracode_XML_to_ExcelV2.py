    ...
    for severity in myroot.findall("{https://www.veracode.com/schema/reports/export/1.0}severity/{https://www.veracode.com/schema/reports/export/1.0}category/{https://www.veracode.com/schema/reports/export/1.0}cwe/{https://www.veracode.com/schema/reports/export/1.0}staticflaws/"):
        ...
        for i in dic:
            ...
            if (i=="sourcefilepath"):
                ...
                worksheet.write(row, col, issueid1)
                worksheet.write(row, col + 1, cate1)

                criticalcol = workbook.add_format({'bg_color': 'C00000'})
                highcol = workbook.add_format({'bg_color': 'FF0000'})
                mediumcol = workbook.add_format({'bg_color': 'FFC000'})
                lowcol = workbook.add_format({'bg_color': '00B050'})
                infocol = workbook.add_format({'bg_color': '4F81BD'})
                if (severity1 == "Critical"):
                    worksheet.write(row, col + 2, severity1,criticalcol)
                elif (severity1 == "High"):
                    worksheet.write(row, col + 2, severity1,highcol)
                elif (severity1 == "Medium"):
                    worksheet.write(row, col + 2, severity1,mediumcol)
                elif (severity1 == "Low"):
                    worksheet.write(row, col + 2, severity1,lowcol)
                elif (severity1 == "Informational"):
                    worksheet.write(row, col + 2, severity1,infocol)    
